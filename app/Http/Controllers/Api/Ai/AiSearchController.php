...
class AiSearchController extends Controller
{
    ...
    public function searchProjects(Request $request): JsonResponse
    {
        ...
        $unresolved = [];
        if ($request->filled('city') && ! $cityId) {
            $unresolved[] = "city: \"{$request->city}\"";
        }
        if ($request->filled('state') && ! $stateId) {
            $unresolved[] = "state: \"{$request->state}\"";
        }
        if ($request->filled('developer') && ! $developerId) {
            $unresolved[] = "developer: \"{$request->developer}\"";
        }
        if ($request->filled('project_type') && ! $projectTypeId) {
            $unresolved[] = "project_type: \"{$request->project_type}\"";
        }
        ...
        if ($hasUnitFilter) {
            $query->whereHas('units', function ($sub) use ($request) {
                if ($request->filled('min_price')) {
                    $sub->where('unit_price', '>=', $request->min_price);
                }
                if ($request->filled('max_price')) {
                    $sub->where('unit_price', '<=', $request->max_price);
                }
                if ($request->filled('min_area')) {
                    $sub->where('unit_area', '>=', $request->min_area);
                }
                if ($request->filled('max_area')) {
                    $sub->where('unit_area', '<=', $request->max_area);
                }
                if ($request->filled('bedrooms')) {
                    $sub->where('beadrooms', '>=', $request->bedrooms);
                }
                if ($request->filled('bathrooms')) {
                    $sub->where('bathrooms', '>=', $request->bathrooms);
                }
                if ($request->filled('sale_type')) {
                    $sub->where('sale_type', $request->sale_type);
                }
            });
        }
    ...
    public function searchUnits(Request $request): JsonResponse
    {
        ...
        if ($request->filled('project') && ! $projectId) {
            $unresolved[] = "project: \"{$request->project}\"";
        }
        ...
        if ($request->filled('case')) {
            $query->where('case', $request->case);
        }
        if ($request->filled('bedrooms')) {
            $query->where('beadrooms', '>=', $request->bedrooms);
        }
        if ($request->filled('bathrooms')) {
        ...
        if ($request->filled('min_area')) {
            $query->where('unit_area', '>=', $request->min_area);
        }
        if ($request->filled('max_area')) {
            $query->where('unit_area', '<=', $request->max_area);
        }
        ...
        if ($cityId || $stateId || $developerId || $projectTypeId) {
            $query->whereHas('project', function ($sub) use ($cityId, $stateId, $developerId, $projectTypeId) {
                ...
                if ($stateId) {
                    $sub->where('state_id', $stateId);
                }
                if ($developerId) {
                    $sub->where('developer_id', $developerId);
                }
                if ($projectTypeId) {
                    $sub->where('project_type_id', $projectTypeId);
                }
            });
        }
