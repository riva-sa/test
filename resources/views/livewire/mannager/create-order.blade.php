            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-lg border border-amber-200 shadow-sm">
                    <div class="p-4">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" wire:model.live="isWaitingList" class="sr-only peer">
                            <div class="relative w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-2 peer-focus:ring-amber-300 peer-checked:bg-amber-500 transition-colors">
                                <div class="absolute top-0.5 right-[2px] bg-white border border-gray-300 rounded-full h-5 w-5 transition-all peer-checked:translate-x-[-20px]"></div>
                            </div>
                            <span class="mr-3 text-sm font-medium text-gray-700">إنشاء الطلب في قائمة الانتظار</span>
                        </label>
                        <p class="text-xs text-gray-500 mt-2">عند التفعيل يتم إنشاء الطلب في قائمة الانتظار بدون اختيار مشروع أو وحدة</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 shadow-sm">
                    ...
                    @else
                        <div class="p-4 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            <div>
                                <label for="waiting_list_unit_type" class="block text-sm font-medium text-gray-700 mb-1">اختر نوع الوحدة <span class="text-red-500">*</span></label>
                                <select id="waiting_list_unit_type" wire:model.live="waiting_list_unit_type" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5">
                                    <option value="">-- اختر نوع الوحدة --</option>
                                    @foreach($waitingListUnitTypes as $value => $label)
                                        <option value="{{ $value }}">{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('waiting_list_unit_type') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="waiting_list_budget" class="block text-sm font-medium text-gray-700 mb-1">الميزانية</label>
                                <input type="text" id="waiting_list_budget" wire:model.live="waiting_list_budget" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" placeholder="مثال: 1,500,000 SAR">
                                @error('waiting_list_budget') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div class="md:col-span-2">
                                <label for="waiting_list_location" class="block text-sm font-medium text-gray-700 mb-1">الموقع</label>
                                <input type="text" id="waiting_list_location" wire:model.live="waiting_list_location" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" placeholder="المدينة أو المنطقة المطلوبة">
                                @error('waiting_list_location') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div class="md:col-span-2">
                        ...
                        @if($PurchaseType === 'Installment')
                            <div class="p-4 border-t border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 bg-slate-50/50">
                                ...
                                <div>
                                    ...
                                    <input type="text" wire:model.live="bank_employee_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" placeholder="اختياري">
                                    @error('bank_employee_name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">رقم تواصل موظف البنك</label>
                                    <input type="text" wire:model.live="bank_employee_phone" dir="ltr" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" placeholder="اختياري">
                                    @error('bank_employee_phone') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                                </div>
                            </div>
                        @endif
                    </div>
