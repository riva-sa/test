@php
    ...
    if ($order->assignedSalesUser) {
        $users->push([
            'user' => $order->assignedSalesUser,
            'label' => 'مندوب معيّن (توزيع)',
    ...
    foreach ($order->permissions as $permission) {
        ...
        $roleLabel = match (true) {
            $user->hasRole('sales_manager') => 'مدير مبيعات',
            $user->hasRole('sales') => 'مندوب مبيعات',
            $user->hasRole('follow_up') => 'متابعة',
            default => '',
        };

        $users->push([
            'user' => $user,
            'label' => $roleLabel . ' له صلاحية الوصول',
            'color' => 'text-gray-500'
        ]);
    }

    $users = $users->unique(fn ($item) => $item['user']->id)->values();
@endphp
<div>
    <div class="px-2 py-4 sm:px-4 bg-white">
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-100 mb-3">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                <div>
                    <div class="flex items-center gap-3">
                        ...
                        <span class="px-3 py-1 text-sm rounded-full" style="background-color: {{ $order->statusColor() }}20; color: {{ $order->statusColor() }}">
                            {{ $order->statusLabel() }}
                        </span>
                        @if ($this->isDelayed())
            ...
            <div class="mt-4 pt-4 border-t border-gray-100">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <div class="flex items-center">
                        <div class="relative">
                            <span class="inline-block h-8 w-8 rounded-full bg-indigo-100 text-indigo-800 flex items-center justify-center font-medium">
                                {{ substr($order->assignedSalesUser->name, 0, 1) }}
                            </span>
                        </div>
                        <div class="mr-2">
                    ...
                    @foreach ($users as $item)
                        <div class="flex items-center">
                            <div class="relative">
                                ...
                                @if($item['user']->hasRole('sales_manager'))
                                    <span class="absolute -bottom-1 -right-1 bg-blue-500 rounded-full p-1 border-2 border-white"></span>
                                @elseif($item['user']->hasRole('sales'))
                                    <span class="absolute -bottom-1 -right-1 bg-green-500 rounded-full p-1 border-2 border-white"></span>
                                @elseif($item['user']->hasRole('follow_up'))
                                    <span class="absolute -bottom-1 -right-1 bg-yellow-500 rounded-full p-1 border-2 border-white"></span>
                                @endif
                            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                    ...
                    <div class="px-4 py-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div class="bg-gray-50 p-4 rounded-lg">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    @if($order->project?->slug)
                                        <a href="{{ route('frontend.projects.single', ['slug' => $order->project->slug]) }}" target="_blank" class="text-primary-600 hover:text-primary-800">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg relative">
                                ...
                                @if($isEditingUnitCase && $this->order->unit)
                                    <div class="mt-1">
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg">
                                ...
                                <p class="text-sm font-medium text-gray-900">{{ $order->purchaseTypeLabel() }}</p>
                            </div>

                            <div class="bg-gray-50 p-4 rounded-lg">
                                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">الغرض من الشراء</p>
                                <p class="text-sm font-medium text-gray-900">{{ $order->purchasePurposeLabel() }}</p>
                            </div>
            ...
            <div class="rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-6">
                <!-- بيانات موظف البنك (التمويل) -->
                <div class="">
                    <div class="px-4 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                        <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-2 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                            </svg>
